...
/// </summary>
/// <remarks>
/// The Avalonia twin of windows/Tokendial.App/Install/InstallAssistant.cs, signature for signature. What
/// differs is the script it writes - a POSIX shell one, mode 700 - and that a platform may simply have no
/// recipe here, in which case there is nothing to offer rather than something wrong.
/// </remarks>
public sealed class InstallAssistant : IDisposable
{
    ...
    public InstallAssistant(IReadOnlyList<IUsageProvider> providers) => this.providers = providers;
    ...
    /// <summary>Whether a terminal exists to run anything in. Without one the sheet offers the command to copy.</summary>
    public bool HasTerminal => TerminalRunner.Available is not null;

    /// <summary>
    /// Where the provider's tool stands right now, read from the disk: a profile beyond the default only
    /// exists once its tool is installed.
    /// </summary>
    public InstallState State(ProviderSummary summary)
    {
        var provider = providers.First(p => p.Id == summary.Id);
        var installed = Platform(summary.Id) is not PlatformRecipe platform
                        || ProviderFamily.IsProfile(summary.Id)
                        || locator.IsInstalled(platform.Detect);
    ...
    /// <summary>This platform's recipe, with the sign-in a profile actually needs, or null when it has none.</summary>
    public PlatformRecipe? Platform(string providerId)
    {
        if (Recipe(providerId)?.Here is not PlatformRecipe platform) return null;
        if (providers.FirstOrDefault(p => p.Id == providerId) is IProfiled profiled
            && ProviderFamily.IsProfile(providerId)
            && platform.SignIn is SignInStep signIn)
    ...
    public void Run(string providerId, InstallAction action)
    {
        ...
        if (Recipe(providerId) is not InstallRecipe recipe || Platform(providerId) is not PlatformRecipe platform) return;

        var path = InstallScript.WriteShell(scripts, providerId, InstallScript.Shell(provider.DisplayName, recipe, platform, action));
        if (watchers.Remove(providerId, out var previous)) previous.Dispose();
        outcomes.Remove(providerId);

        var watcher = new InstallWatcher(providerId, () => locator.IsInstalled(platform.Detect),
            () => provider.Account() is not null, waitsAfterTerminal: platform.Kind == InstallKind.App);
        watcher.Advanced += _ => Dispatcher.UIThread.Post(() => Changed?.Invoke());
        watcher.Completed += outcome => Dispatcher.UIThread.Post(() =>
        {
