...
- [x] Blog for vibecoders in `site/src/content/blog` (en + es): sections (basics, save tokens, tools, design, install, news), credited figures, per-post covers and Sources lists; RSS, related posts, landing block
...
- [ ] Feature ideas priced against the current code in `tasks/feature-ideas.md`: a webhook alert sink and `tokendial status --json` are cheap and need no stored history; anything report-shaped first needs the decision to persist samples, which the app deliberately does not do today
- [ ] Candidate providers researched and parked in `tasks/candidate-providers.md`: Augment Code is fully specified from the vendor CLI and blocked only on one validated response (no free tier, cheapest plan $20/mo, no machine here); Windsurf rejected while it is being renamed to Devin Desktop and exposes no personal usage endpoint
